...
import { ArrowLeft, BadgeCheck, Briefcase, CalendarCheck, CheckCircle2, MapPin, Phone, ShieldCheck, Sparkles, Star, Wrench, User } from 'lucide-react';
...
const WorkerProfile = () => {
  ...
  const [workerData, setWorkerData] = useState(location.state?.worker || null);
  const [loading, setLoading] = useState(!location.state?.worker);
  const [error, setError] = useState('');

  useEffect(() => {
    const fetchWorker = async () => {
      if (id === 'me' || (user?.worker?.id && id === user.worker.id.toString())) {
        setLoading(false);
        return;
      }
  ...
  const sourceData = isOwnProfile ? (user?.worker || user) : (workerData || {});

  let workerName = isOwnProfile ? (user?.name || sourceData?.name || sourceData?.user?.name || 'Rajesh Kumar') : (sourceData?.name || sourceData?.user?.name || 'Rajesh Kumar');
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-[1.5rem] border border-gray-100 shadow-sm overflow-hidden">
              <div className="p-6 sm:p-8 flex flex-col sm:flex-row gap-6 items-start sm:items-center">
                <img src={worker.image} alt={worker.name} className="w-28 h-28 sm:w-32 sm:h-32 rounded-2xl object-cover border border-gray-100 shadow-sm shrink-0" />
                <div className="flex-1">
                  ...
                  <p className="text-base font-bold text-emerald-600 mt-1">{worker.skill}</p>
                  
                  <div className="flex flex-wrap gap-4 mt-4 text-sm font-semibold text-slate-500">
                    <span className="flex items-center gap-1.5"><MapPin className="w-4 h-4 text-slate-400" />{worker.location}</span>
                    <span className="flex items-center gap-1.5"><Briefcase className="w-4 h-4 text-slate-400" />{worker.completedJobs} jobs</span>
                    <span className="flex items-center gap-1.5"><Star className="w-4 h-4 text-amber-500 fill-amber-500" />{worker.rating} rating</span>
                  </div>
                </div>
              </div>

              <div className="bg-slate-50 border-t border-gray-100 p-4 sm:px-8 grid grid-cols-3 gap-4">
                {[
                  { label: 'Identity Verified', icon: ShieldCheck, color: 'text-emerald-600' },
                  { label: 'Background Clear', icon: CheckCircle2, color: 'text-blue-600' },
                  { label: 'Top Rated', icon: Star, color: 'text-amber-600' },
            ...
            <div className="bg-white rounded-[1.5rem] border border-gray-100 shadow-sm p-6 sm:p-8">
              ...
              <p className="text-sm text-slate-600 font-medium leading-relaxed mt-3">{worker.bio}</p>
